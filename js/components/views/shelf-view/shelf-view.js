...
export class ShelfView extends AppElement {
  ...
  _recordTpl(p, i) {
    ...
    const grooves = grooveTexture(p.id || p.name, 9, 30, 54)
      .map((g) => `<circle cx="60" cy="60" r="${g.r}" fill="none" stroke="var(--color-neutral-700)" stroke-width="1" opacity="${g.opacity}"${g.gap ? ' stroke-dasharray="2 4"' : ''}></circle>`).join('');
    ...
    return `
      <div class="rec" style="animation-delay:${Math.min(i * 40, 300)}ms">
        <svg class="mini" viewBox="0 0 120 120" aria-hidden="true">
          <circle cx="60" cy="60" r="56" fill="none" stroke="var(--ink)" stroke-width="2"></circle>
          ${grooves}
